...
/**
 * Posición (índice, base 0) que usa `quart()` del motor para cada percentil,
 * sobre una serie de tamaño `n` ya ordenada de menor a mayor:
 * floor(percentil × (n − 1)). Se reproduce aquí para poder marcar en la tabla
 * de trazabilidad exactamente qué comparable produjo cada percentil — la
 * misma fórmula, no una aproximación.
 */
function indicesPercentiles(n) {
  if (!n) return {};
  return {
    p25: Math.floor(0.25 * (n - 1)),
    med: Math.floor(0.5 * (n - 1)),
    p75: Math.floor(0.75 * (n - 1)),
  };
}

/** Filas de la tabla que muestra, comparable por comparable, de dónde sale el rango. */
function filasTrazabilidadRango(serie) {
  if (!serie || !serie.length) {
    return [['Ninguna comparable activa entró al cálculo del rango (revise el filtro Todas/Internacionales/Nacionales y que tengan PLI ajustado calculable).']];
  }
  const idx = indicesPercentiles(serie.length);
  const marca = (i) => [
    i === idx.p25 ? 'Percentil 25' : '',
    i === idx.med ? 'Mediana' : '',
    i === idx.p75 ? 'Percentil 75' : '',
  ].filter(Boolean).join(' / ');

  const filas = [['Posición', 'Razón Social', 'PLI Ajustado', 'Percentil que marca']];
  serie.forEach((row, i) => {
    filas.push([i + 1, row.name || '', conFormato(row.adjustedPli, pctf), marca(i)]);
  });
  return filas;
}
...
function hojaResumen(datos) {
  ...
  const { stats, activeCount, adjustment, serie } = rango;
  ...
  const filas = [
    ...
    [],
    ['De dónde sale el rango: comparables activas ordenadas de menor a mayor PLI ajustado'],
    ...filasTrazabilidadRango(serie),
    [],
