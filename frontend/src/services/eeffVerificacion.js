...
export function verificarEeff(lectura, { anioEstudio } = {}) {
  ...
  /* Se leen de `campos` y no de la lectura cruda: si la comprobación de presencia literal
     descartó alguna, la utilidad no puede calcularse con ella. */
  const ventas = campos.t_s;
  const costo = campos.t_c === null ? null : egreso(campos.t_c);
  const gastosVentas = egreso(cotejo.gastos_ventas);
  const gastosAdmon = egreso(cotejo.gastos_administracion);

  /* Con uno solo de los dos se sigue adelante —hay estados que no desglosan gastos de
     ventas porque los llevan todos a administración— pero si faltan los dos no hay
     gastos operativos que calcular. */
  const gastos = (gastosVentas === null && gastosAdmon === null)
    ? null
    : (gastosVentas || 0) + (gastosAdmon || 0);

  const uop = (ventas !== null && costo !== null && gastos !== null)
    ? ventas - costo - gastos
    : null;
  ...
  if (utilidadBrutaLeida !== null && ventas !== null && costo !== null
      && !cuadra(utilidadBrutaLeida, ventas - costo, ventas)) {
    advertencias.push({
      tipo: 'utilidad-bruta-no-cuadra',
      mensaje: `La utilidad bruta que imprime el documento (${fmtCop(utilidadBrutaLeida)}) no es `
        + `ingresos ${fmtCop(ventas)} − costo ${fmtCop(costo)} = ${fmtCop(ventas - costo)}. `
