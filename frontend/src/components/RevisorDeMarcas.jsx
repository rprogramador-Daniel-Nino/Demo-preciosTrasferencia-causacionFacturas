...
export default function RevisorDeMarcas({ marcas, onConfirmar, onCancelar }) {
  ...
  const cambiarCampo = (i, campo) =>
    setLista(prevLista => prevLista.map((m, j) => (j === i ? { ...m, campo } : m)));

  const quitar = (i) => setLista(prevLista => prevLista.filter((_, j) => j !== i));
    ...
    <div className="bg-white dark:bg-[#0c0c0f] border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
      ...
      <div className="max-h-[420px] overflow-y-auto space-y-2">
        {lista.map((m, i) => {
          ...
          const marcaId = `${m.fragmento}|${m.campo}|${ocurrencia}`;
          return (
            <div key={marcaId} className="flex items-start gap-3 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2">
              ...
              <div className="flex items-start gap-2">
                <div>
                  <label htmlFor={`marca-${i}`} className="text-xs text-zinc-600 dark:text-zinc-400 block mb-1">
                    Campo
                  </label>
                  <select
                    id={`marca-${i}`}
                    ...
                    aria-label={`Campo para fragmento: ${m.fragmento.slice(0, 30)}...`}
                  >
                    {VOCABULARIO.map((v) => (
                ...
                <button
                  ...
                  title={`Eliminar marca: ${m.fragmento.slice(0, 30)}...`}
